...
# Project Context Reader 原子Skill

## 概述
读取项目上下文文件 `./.claude/project-context.json`，该文件由 scan-object-info skill 生成或由用户手动提供。本skill **不自动扫描任何项目文件**，完全依赖外部上下文输入。
...
## 输入
前一个skill（requirement-evaluation）的输出结果
...
**输入**：
```
./.claude/project-context.json 存在
```

**处理过程**：
1. 读取文件
2. 验证字段：frontend_framework、ui_library、project_structure
3. 所有字段完整
4. 记录来源：scan-object-info
...
**输入**：
```
./.claude/project-context.json 不存在
用户选择：跳过，使用默认配置
```
...
## project-context.json 完整格式
```json
{
  "frontend_framework": "vue3 | react18 | angular15 | svelte4 | vanilla",
  "ui_library": {
    "name": "element-plus | antd | mui | naive-ui | custom",
    "version": "2.2.31",
    "components": ["el-form", "el-input", "el-button", "el-table"]
  },
  "project_structure": {
...
```
方式1（推荐）：
用户执行 scan-object-info → 生成 project-context.json
↓
用户执行 requirement-generator → project-context-reader 自动读取

方式2（手动）：
用户手动创建 project-context.json
↓
用户执行 requirement-generator → project-context-reader 读取

方式3（跳过）：
用户执行 requirement-generator → 检测无上下文
↓
用户选择跳过 → 使用默认配置
```
...
## 原子skill位置
./.claude/skills/requirement-generator/atomic-skills/project-context-reader/SKILL.md
