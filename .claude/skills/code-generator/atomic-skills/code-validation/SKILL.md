...
description: 验证生成的代码语法、规范、功能完整性。优先验证 requirements.json 中的 test_cases，并继承 quality 评估。在 code-generation 或 module-integration 之后执行。v1.3 整合韬定律（τ 控制、技能栈叠）。
---

# Code Validation 原子Skill v1.3（τ 增强版）
...
| 键名 | 类型 | 说明 |
|------|------|------|
| generated_code / integrated_project | required_keys | 从 task_skill.md 读取（由 code-generation/module-integration 写入，优先从上下文读取） |
| tech_stack | required_keys | 从 task_skill.md 读取（由 tech-stack-detection 写入，优先从上下文读取） |
| validation_result | output_keys | 语法验证 + 规范检查 + 测试用例 + quality 继承结果 |

### 执行日记写入（必须）
本 atomic skill 执行完成后，必须将 output_keys 写入 `task_skill.md` 的技能输出区域，供下游 skill 通过 Skill Stacking TSV 读取。

### Task Folding 标记
**[核心·可折叠]** — 本 skill 可与 module-integration 合并为 Group C 折叠组（场景3），也可与 documentation-update 合并为 Group D 折叠组（所有场景），τ_remaining < 20% 时优先折叠。