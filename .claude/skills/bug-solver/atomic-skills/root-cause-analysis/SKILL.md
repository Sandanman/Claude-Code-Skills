---
name: root-cause-analysis
description: 基于代码分析报告和错误日志，定位bug的根本原因，明确导致问题的代码行和逻辑缺陷。在代码分析后执行
---

# Root Cause Analysis 原子Skill

## 概述
基于代码分析报告和错误日志，定位bug的根本原因，明确导致问题的代码行和逻辑缺陷。

## 核心能力
- 整合错误日志与代码分析结果
- 定位具体代码行和逻辑缺陷
- 分析错误触发条件
- 确定根本原因而非表面现象
- 输出根因定位报告

## 输入
code-analysis输出的代码分析报告

## 输出
根因定位报告，包含以下内容：

```markdown
# 根因定位报告

## 分析目标
[根据代码分析报告确定的分析目标]

## 根本原因
[清晰明确的根本原因陈述，格式："由于...导致..."]

## 证据链
1. [证据1：错误日志内容]
2. [证据2：相关代码段]
3. [证据3：调用关系]
4. [证据4：触发条件]

## 详细分析
- **错误日志分析**：
  [错误日志的详细分析，解释其含义]

- **代码行为分析**：
  [相关代码执行流程的详细分析]

- **触发条件分析**：
  [什么情况下会触发此错误？]

- **为什么是根本原因**：
  [为什么这个原因是根本原因，而不是表面现象？]

## 与其他可能性的对比
| 可能性 | 描述 | 是否可能 | 理由 |
|--------|------|----------|------|
| [可能性1] | [描述] | [是/否] | [理由] |
| [可能性2] | [描述] | [是/否] | [理由] |
| [可能性3] | [描述] | [是/否] | [理由] |

## 根因确认
- [ ] 是否能稳定复现？
- [ ] 是否在所有环境都出现？
- [ ] 是否有其他因素影响？
- [ ] 是否与最近的代码变更相关？

## 最终结论
[最可能的根本原因，用一句话总结]

## 建议的修复方向
[基于根因的修复建议，如：添加对象存在性检查、修正导入路径、增加初始化等待等]
```

## 执行逻辑
1. 整合错误日志和代码分析报告
2. 构建从错误日志到代码行的证据链
3. 分析错误触发的完整条件
4. 区分根本原因与表面现象
5. 对比其他可能原因
6. 输出根因定位报告

## 依赖关系
- 依赖：code-analysis（必须有完整的代码分析报告）

## 完成标准
1. 根本原因明确陈述（"由于...导致..."格式）
2. 至少提供4个证据支持根因
3. 包含与其他可能性的对比分析
4. 区分根本原因与表面现象
5. 输出完整的根因定位报告

## 错误处理
- 如果证据不足，标记为"需要更多信息"
- 如果存在多个可能原因，列出所有可能性并评估
- 如果无法确定根本原因，暂停并请求用户确认

## 示例

**输入：**
code-analysis输出的报告中包含：
- 错误日志：TypeError: Cannot read property 'login' of undefined
- 可疑代码段：src/views/Login/index.vue:50（调用userAPI.login）
- 导入路径问题：src/views/Login/index.vue:28（import userAPI from '@/utils/utils'）

**输出：**
```markdown
# 根因定位报告

## 分析目标
定位登录功能中"TypeError: Cannot read property 'login' of undefined"的根本原因

## 根本原因
由于在Login组件中错误地导入了userAPI模块（导入路径为'@/utils/utils'而非'@/api/user.js'），导致userAPI对象为undefined，进而当调用userAPI.login时触发了Cannot read property 'login' of undefined错误。

## 证据链
1. **错误日志**：TypeError: Cannot read property 'login' of undefined
2. **相关代码段**：src/views/Login/index.vue:50 - const response = await userAPI.login({ ... })
3. **调用关系**：loginEvent -> userAPI.login -> 未定义对象
4. **触发条件**：当userAPI对象被错误导入为undefined时，在点击登录按钮后立即触发

## 详细分析
- **错误日志分析**：
  "Cannot read property 'login' of undefined" 表明试图访问undefined对象的login属性，这是典型的JavaScript引用错误。

- **代码行为分析**：
  Login组件第28行：import userAPI from '@/utils/utils' —— 该路径指向一个工具模块，而非API模块。
  该模块导出的是通用工具函数，不包含login方法，因此userAPI为undefined。
  第50行尝试调用userAPI.login()时，由于userAPI是undefined，触发错误。

- **触发条件分析**：
  此错误在以下条件下触发：
  - 用户点击登录按钮
  - userAPI模块导入路径错误
  - 用户输入有效用户名和密码
  - 页面加载完成

- **为什么是根本原因**：
  这个问题是根本原因，因为：
  - 它解释了为什么会出现"undefined"（导入路径错误）
  - 它解释了为什么是"login"属性（API模块应该提供login方法）
  - 它是其他所有现象的根源
  - 如果修复导入路径，错误将完全消失

## 与其他可能性的对比
| 可能性 | 描述 | 是否可能 | 理由 |
|--------|------|----------|------|
| userAPI模块未导出login方法 | userAPI模块缺少login方法 | 否 | code-analysis确认userAPI模块有正确的login方法定义 |
| API服务端故障 | 后端API返回错误 | 否 | 错误是前端TypeError，不是HTTP错误 |
| 异步加载问题 | userAPI异步加载未完成 | 否 | 导入语句是静态导入，不是异步加载 |
| 导入路径错误 | 导入了错误的模块 | 是 | 导入路径'@/utils/utils'指向了错误的模块，而正确的路径应为'@/api/user.js' |
| 编译打包问题 | 构建过程错误 | 否 | 其他功能正常，仅此问题，且代码逻辑清晰 |

## 根因确认
- [x] 是否能稳定复现？
- [x] 是否在所有环境都出现？
- [ ] 是否有其他因素影响？
- [x] 是否与最近的代码变更相关？（可能因重构时修改了导入路径）

## 最终结论
根本原因是Login组件中userAPI模块的导入路径错误，导致userAPI对象为undefined。

## 建议的修复方向
1. 修正导入路径：将src/views/Login/index.vue第28行的
   `import userAPI from '@/utils/utils'`
   改为
   `import userAPI from '@/api/user.js'`
2. 添加防御性检查：在调用userAPI.login前添加对象存在性检查
   ```js
   if (userAPI && typeof userAPI.login === 'function') {
       await userAPI.login(...)
   } else {
       console.error('userAPI is not properly initialized')
   }
   ```
3. 建议在构建时添加导入路径验证
```

## 注意事项
- 聚焦根本原因，避免表面现象
- 使用明确的"由于...导致..."格式
- 提供完整的证据链支持
- 区分根本原因与表面现象

## 原子skill位置
./.claude/skills/bug-solver/atomic-skills/root-cause-analysis/SKILL.md