---
name: code-analysis
description: 基于问题识别阶段的报告，分析相关代码文件，识别潜在的代码问题、异常处理、边界条件等，为根因定位提供依据。在问题识别后执行。v1.3 整合韬定律（τ 控制、技能栈叠）。
---

# Code Analysis 原子Skill v1.3（τ 增强版）
...
| 键名 | 类型 | 说明 |
|------|------|------|
| `bug_report` | required_keys | 从 task_skill.md 读取，结构化问题描述 |
| `analysis_result` | output_keys | 可疑代码段列表 |

### 执行日记写入（必须）
本 atomic skill 执行完成后，必须将 output_keys 写入 `task_skill.md` 的技能输出区域，供下游 skill 通过 Skill Stacking TSV 读取。

### 折叠标记
[可折叠] - 可与 root-cause-analysis 合并（moderate 复杂度，文件 < 3）。